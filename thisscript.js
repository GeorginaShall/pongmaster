...
function movePaddle() {
  ...
  if (player.x >= mywidth - player.image.width) {
...
function hitWall() {
  ...
  if (ball.x + ball.image.width / 2 > mywidth) {
    //right wall
    xSpeed = -xSpeed;
...
function moveCpu() {
  ...
  if (cpu.x + cpu.image.width / 2 < ball.x - ball.image.width) {
    ...
    // if((cpu.x - ((cpu.image.width)/2)) == 0) { //left wall
    //   cpu.x = cpuSpeed;; } }
  } else if (cpu.x + cpu.image.width / 2 > ball.x + ball.image.width) {
    // ||((cpu.x + (cpu.image.width)) > mywidth)
    cpu.x = cpu.x - cpuSpeed;
  }

  if (cpu.x >= mywidth - cpu.image.width) {
    //stop the paddle from going out of canvas *1.5
